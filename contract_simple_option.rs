...
/*
    a. Concepts in the code: 
        Cloning the value at info.sender into the state.creator and state.owner, 
        Utilizing different custom contract errors spanning from superclass crate::error::ContractError
        Polymorphism: Utilizing the same BankMsg::Send() fn signature to perform two different functions

    b. Organization in the code
        Code is organized with utilizing many classes from crate superclass in regards to defining
        custom msgs, responses.  Utilization of tests module to do unit testing of many of the functions,
        basic assert_eq! macros and .unwrap(), .unwrap_err() for testing 
        
    c. Contract is doing? Mechanism?
        Contract is supporting a basic option transfering and execution framework, where the different ExecuteMsgs
        include transferring, actually executing the option, and burning the option when necessary.  
        Depending on the Execute Function depend on several different validation checks, including that the sender of the msg is the owner,
        the contract hasn't expired by examining the block height, ensures proper counteroffer/collateral, etc.

    d. How could it be better? More efficient?
        Interesting way to implement testing with the .unwrap() to assert success and .unwrap_err() for asserting error.
        I think there can be better testing methods, but for a basic testing method it seems sufficient for simplicity.  
        The different validation checks can be improved, but based on simple implementation of the methods.
...
/*
    execute_execute checks with error handling that:
        1. The owner matches the sender of the message -> Unauthorized
        2. The option is in fact valid, and not expired
        3. The state's counter_offer matches the message info's funds
    then the counter_offer is released to the creator
    then the collateral is released to the sender
*/
...
/*
    execute_burn checks first with error handling that:
        1. the option is in fact expired
        2. funds are actually empty
    then construct message to release the state's collateral to the creator
    then delete/burn the option
*/
